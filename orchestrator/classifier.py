...
class FileClassifier:
    ...
    def classify_file(self, filepath: str) -> str:
        ...
        try:
            ...
            if not extension_category and metadata.get('mime_type') in ['text/plain', 'application/octet-stream']:
                final_category = 'unknown'
            # For specific MIME types that are clearly identifiable, prefer MIME type
            elif mime_category and metadata.get('mime_type') not in ['text/plain', 'application/octet-stream']:
            ...
            self.logger.info(f"File classified: {filepath} -> {final_category} "
