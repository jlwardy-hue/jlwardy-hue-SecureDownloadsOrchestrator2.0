...
from orchestrator.pipeline import PathValidationError, UnifiedFileProcessor
...
class TestPathValidation:
    ...
    def processor_config(self):
        ...
        return {
            ...
            "categories": {"documents": {"extensions": [".txt", ".pdf"]}},
    ...
    def processor(self, processor_config):
        ...
        Path(processor_config["directories"]["source"]).mkdir(
            parents=True, exist_ok=True
        )
        Path(processor_config["directories"]["destination"]).mkdir(
            parents=True, exist_ok=True
        )
        Path(processor_config["directories"]["quarantine"]).mkdir(
            parents=True, exist_ok=True
        )
        return processor
    ...
    def test_path_validation_in_process_file(self, processor):
        ...
        try:
            ...
            try:
                ...
                assert (
                    "Path validation failed" in result.error
                    or "File does not exist" in result.error
                )
