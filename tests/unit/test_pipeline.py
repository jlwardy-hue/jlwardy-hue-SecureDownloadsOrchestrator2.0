...
class TestUnifiedFileProcessor:
    ...
    def test_processor_initialization(self, processor, pipeline_config):
        ...
        assert processor.destination_dir == Path(pipeline_config["directories"]["destination"])
    ...
    def test_process_simple_text_file(self, processor, temp_directories):
        ...
        assert "document" in result.final_path  # Should contain "document" (singular from config)
    ...
    @patch('orchestrator.pipeline.subprocess.run')
    def test_security_scanning_infected_file(self, mock_run, processor, temp_directories):
        ...
        mock_run.return_value = Mock(
            returncode=1, 
            stdout="file.txt: TestVirus FOUND", 
            stderr=""
        )
    ...
    def test_archive_extraction(self, processor, temp_directories):
        ...
        assert len(result.metadata["extracted_files"]) >= 0  # Files should be extracted and processed
    ...
    def test_ocr_metadata_extraction(self, mock_image, mock_tesseract, processor, temp_directories):
        """Test OCR metadata extraction from images."""
        # Mock tesseract to return sample text
        mock_tesseract.image_to_string.return_value = "Invoice from: john@example.com\nDate: 01/15/2024\nAmount: $100"
